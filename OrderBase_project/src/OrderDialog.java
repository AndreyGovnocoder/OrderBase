...
class OrderDialog
{
    ...
    void show(Stage primaryStage)
    {
        ...
        if(_editOrder) _orderDialogStage.setTitle("Редактирование заказа");
        else _orderDialogStage.setTitle("Создание нового заказа");
    ...
    private VBox getCenter()
    {
        ...
        _paymentCombobox = new ComboBox<>(FXCollections.observableArrayList("Оплачено", "Не оплачено", "50%"));
        ...
        {
            if(_clientCombobox.isShowing()) _clientCombobox.hide();
            ...
            {
                _clientCombobox.getItems().clear();
                _clientCombobox.setItems(FXCollections.observableArrayList(MainInterface.getAllClients()));
                _clientCombobox.show();
            }
        });
        ...
        createClientMenuItem.setOnAction(event ->
        {
            ...
            clientDialog.show(_orderDialogStage);
            if(clientDialog._ok)
            {
                ...
                MainInterface.getAllClients().add(clientDialog.get_client());
            }
        });
        ...
        if(_editOrder)
        {
            ...
            _clientTextField.setText(MainInterface.getClient(_order.get_client()).get_name());
            _clientCombobox.setValue(MainInterface.getClient(_order.get_client()));
            _paymentCombobox.setValue(_order.get_payment());
            _amountTextField.setText(_order.get_amount());
            _managerCombobox.setValue(MainInterface.getStaff(_order.get_manager()));
            _designerCombobox.setValue(MainInterface.getStaff(_order.get_designer()));
    ...
    {
        _clientCombobox.getItems().addAll(MainInterface.getAllClients());
    ...
    private void setStaffsCombobox()
    {
        ...
        for(Staff staff : MainInterface.getAllStaffs())
        {
            switch (staff.get_position())
            {
                case "менеджер":
                    _managerCombobox.getItems().add(staff);
                    break;
                case "дизайнер":
                    _designerCombobox.getItems().add(staff);
                    break;
            }
        }
    }
    ...
    ArrayList<Client>  searchClient(String searchClient)
    {
        ...
        for(Client client : MainInterface.getAllClients())
        {
    ...
    private boolean set_order()
    {
        if(checkToMakeOrder())
        {
            ...
            if(_designerCombobox.getSelectionModel().isEmpty()) _order.set_designer(-1);
            else _order.set_designer(_designerCombobox.getValue().get_id());
